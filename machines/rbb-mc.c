...
/**
 * RBB CLIENT-side circuit padding machine on the general circuit.
 */  
  ...
  client_machine->name = "rbb_client";
  ...
  //client_machine->conditions.apply_purpose_mask = circpad_circ_purpose_to_mask(CIRCUIT_PURPOSE_C_GENERAL)|circpad_circ_purpose_to_mask(CIRCUIT_PURPOSE_C_CIRCUIT_PADDING);
...
/******************  [START OF STATES]  *****************/
  ...
  client_machine->states[0].next_state[CIRCPAD_EVENT_NONPADDING_RECV] = 1;
  ...
  client_machine->states[1].next_state[CIRCPAD_EVENT_INFINITY] = 0;
  // +In state three, the infinity bin signals that a fake burst should end
  client_machine->states[2].next_state[CIRCPAD_EVENT_INFINITY] = 0;
  // Stop padding if the maximum padding per burst is reached.
  client_machine->states[2].next_state[CIRCPAD_EVENT_LENGTH_COUNT] = 0;

  // Machine should transition to state three when padding is sent (ie. fake burst is active)
  client_machine->states[1].next_state[CIRCPAD_EVENT_PADDING_SENT] = 2;
  ...
  client_machine->states[1].histogram_len = 2;
  client_machine->states[1].histogram_edges[0] = 0;
  client_machine->states[1].histogram_edges[1] = 1;
  // 10% to start padding
  client_machine->states[1].histogram[0] = 1;
  client_machine->states[1].histogram[1] = 9;
  client_machine->states[1].histogram_total_tokens = 10;
  client_machine->states[1].use_rtt_estimate = 0;
  client_machine->states[1].token_removal = CIRCPAD_TOKEN_REMOVAL_NONE;

  // State Three Histogram definition:
  // +Two bins
  // +Infinity bin indicates not end fake burst
  // +Zero bin continues burst with additional cell
  client_machine->states[2].histogram_len = 2;
  client_machine->states[2].histogram_edges[0] = 0;
  client_machine->states[2].histogram_edges[1] = 1;
  // 50% to continue padding, 50% to end
  client_machine->states[2].histogram[0] = 5;
  client_machine->states[2].histogram[1] = 5;
  client_machine->states[2].histogram_total_tokens = 10;
  client_machine->states[2].use_rtt_estimate = 0;
  client_machine->states[2].token_removal = CIRCPAD_TOKEN_REMOVAL_NONE;
  // Set maximum number of cells per fake burst.
  client_machine->states[2].length_dist.type = CIRCPAD_DIST_UNIFORM;
  client_machine->states[2].start_length = 5;
  client_machine->states[2].max_length = 5;
  client_machine->states[2].length_includes_nonpadding = 0;
...
/*****************************  [END OF STATES]  *****************************/
  ...
  /* [LOGGING] : */
  log_info(LD_CIRC, "[RBB_CLIENT] Registered the RBB client padding machine (%u)", client_machine->machine_num);
